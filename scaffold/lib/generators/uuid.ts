...
export class UUIDGen implements IFieldGenerator {
  type: Type = 'uuid';
  ...
  src_zod_model(name: string): string {
    return `${name}: z.string().uuid({ message: "Invalid UUID" }),`;
  }
  ...
  test_unit_service_search(name: string, amt: number): string[] {
    return [...Array(amt)].map(() => `${name}: '${faker.string.uuid()}',`);
  }
