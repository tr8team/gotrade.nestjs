...
export class EmailGen implements IFieldGenerator {
  type: Type = 'email';
  ...
  src_zod_model(name: string): string {
    return `${name}: z.string().email({ message: "Invalid email address" }),`;
  }
  ...
  test_unit_service_create_model(name: string): string {
    return `${name}: '${faker.internet.email()}',`;
  }
  ...
  test_unit_service_search(name: string, amt: number): string[] {
    return [...Array(amt)].map(() => `${name}: '${faker.internet.email()}',`);
  }
